...
class PretrainedHMM():
    """
        Class PretrainedHMM reads two files, one containing transition probabilities between two NE-pos-tags, the other
        containing emission probs for NE-pos-tags per token. Transition probabilities are stored in a two-dimensional
        dictionary, emission probabilities in a dictionary with an array containing all pos-tag-probabilites per token.

        :param trans_probs:     Path to input transposition-probabilities-file
        :param emi_probs:       Path to input emission-probabilities-file
    """
    ...
    def tag_sent(self,
                 tok_sent: str):
        """
        Viterbi probability is calculated as follows:
            - emission prob for (token(i+1)|pos-tag) * max(transposition prob for (pos_tag_prev | pos_tag_curr)
              * viterbi prob for token(i | pos_tag_prev))
        :param tok_sent: tokenized sentence
        :return: the most probable pos-tag sequence in a tuple and its viterbi probability.
        """
        sent_lst = []
        ...
        # first token can be calculated based on V(0) = 1.0, this won't be changed again.
